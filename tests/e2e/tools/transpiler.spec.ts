...
test.describe('Code Transpiler Tool', () => {
  test('transpiles TypeScript sample and clears editors', { tag: '@smoke' }, async ({ page }) => {
    ...
    await test.step('clears input and output', async () => {
      ...
      expect(inputText.replace('Paste TypeScript or JSX', '').trim()).toBe('')

      const outputText = (await outputEditor.textContent()) ?? ''
      expect(outputText.replace('JavaScript appears here', '').trim()).toBe('')
    })
  })
})
