...
const props = withDefaults(defineProps<{
  ...
  toolId: string
  /** The color that the tool holds when the page opens. */
  baseColor?: string
  /** The token name that the tool holds when the page opens. */
  tokenName?: string
  /** The export format that the tool shows when the page opens. */
  format?: CodeFormat
}>(), { baseColor: '#3b82f6', tokenName: 'brand', format: 'v4' })

useToolSeo(props.toolId)

const { consumeHandoffColor } = useColorHandoff()

const inputColor = ref(props.baseColor)
const colorName = ref(props.tokenName)

// The recipe holds only the enumerated options. The color and the name stay in memory.
const anchorShade = useToolOption<ShadeKey>('anchor', DEFAULT_ANCHOR, props.toolId)
const format = useToolOption<CodeFormat>('format', props.format, props.toolId)
...
const roleShades = useToolOption<Record<string, ShadeKey>>('roles', { ...ROLE_DEFAULTS }, props.toolId)
...
const { error, result } = useLiveTool<TailwindShade[]>(
  () => generateTailwindPalette(inputColor.value, anchorShade.value).map((shade) => {
...
const palette = computed<TailwindShade[]>(() => result.value ?? [])
...
const codeOutput = computed(() => {
  if (palette.value.length === 0)
    return ''
  const name = colorName.value.trim() || props.tokenName
  if (format.value === 'v4') {
...
function handleReset() {
  inputColor.value = props.baseColor
  colorName.value = props.tokenName
...
function handleCopyCssVars() {
  copy(formatAsCssVars(palette.value, colorName.value.trim() || props.tokenName), 'css-vars', 'snippet')
}
